...
| `editable_keys` | `[]` | Optional allowlist — when non-empty, only these keys are editable by UI/API. |
| `interpolation.resolve` | `true` | Resolve `${VAR}` references on `interpolated()` reads. |
| `interpolation.on_undefined` | `'empty'` | `'empty'` substitutes `''`; `'throw'` raises a `ValidationException`. |
...
Two independent lists protect keys at different layers:
...
Both can be extended at runtime — see [Extending](extending.md):

```php
EnvKit::configure()->protectKeys(['STRIPE_SECRET']);
```

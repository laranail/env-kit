...
it('reports a clean file with env:doctor', function () {
    ...
    $this->artisan('laranail::env-kit.doctor')->expectsOutputToContain('No issues found.')->assertExitCode(0);
});

it('flags duplicate keys as an error (exit 3)', function () {
    $this->bindEnv("A=1\nA=2\nB=3\n");

    $this->artisan('laranail::env-kit.doctor')->expectsOutputToContain('defined 2 times')->assertExitCode(3);
});

it('warns on a missing trailing newline (non-fatal)', function () {
    $this->bindEnv('A=1'); // no trailing newline

    $this->artisan('laranail::env-kit.doctor')->expectsOutputToContain('newline')->assertExitCode(0);
});
...
it('runs a custom doctor rule registered via configure()', function () {
    ...
    $this->artisan('laranail::env-kit.doctor')->expectsOutputToContain('FORBIDDEN key present')->assertExitCode(3);
});
...
it('prints the diff with env:diff', function () {
    ...
    $this->artisan('laranail::env-kit.diff', ['against' => $other])
...
it('reports no differences for identical files', function () {
    ...
    $this->artisan('laranail::env-kit.diff', ['against' => $other])->expectsOutputToContain('No differences.')->assertExitCode(0);
});

it('prefixes key-scoped findings with the key name', function () {
    $this->bindEnv("A=\nB=1\n");

    $this->artisan('laranail::env-kit.doctor')

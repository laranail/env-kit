...
it('lists keys missing from the .env relative to .env.example', function () {
    ...
    config(['laranail.env-kit.path' => $dir . '/.env', 'laranail.env-kit.auto_backup' => false]);
...
it('env:check exits non-zero on drift and zero when in sync', function () {
    ...
    $this->artisan('laranail::env-kit.check')->expectsOutputToContain('missing')->assertExitCode(3);

    $this->artisan('laranail::env-kit.sync')->expectsOutputToContain('Added')->assertExitCode(0);
    $this->artisan('laranail::env-kit.check')->expectsOutputToContain('in sync')->assertExitCode(0);
});

it('env:generate produces a value and can write it to a key', function () {
    $this->bindEnv("A=1\n", ['laranail.env-kit.auto_backup' => false]);

    $this->artisan('laranail::env-kit.generate', ['type' => 'hex', '--bytes' => 8])->assertExitCode(0);
    $this->artisan('laranail::env-kit.generate', ['type' => 'app_key', '--set' => 'NEW_KEY'])

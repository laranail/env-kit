...
it('sets a key via the env:set alias and the namespaced name', function () {
    ...
    $this->artisan('laranail::env-kit.set', ['key' => 'NEW', 'value' => 'val'])->assertExitCode(0);
...
it('accepts KEY=VALUE shorthand and rejects the mixed form', function () {
    ...
    $this->artisan('laranail::env-kit.set', ['key' => 'FOO=bar'])->assertExitCode(0);
    expect(EnvDocument::parse(file_get_contents($path))->get('FOO'))->toBe('bar');

    // both KEY VALUE and KEY=VALUE at once → usage error (exit 2)
    $this->artisan('laranail::env-kit.set', ['key' => 'X=1', 'value' => '2'])->assertExitCode(2);
});

it('rejects an invalid key with the validation exit code', function () {
    $this->bindEnv("A=1\n", ['laranail.env-kit.auto_backup' => false]);

    $this->artisan('laranail::env-kit.set', ['key' => '1bad', 'value' => 'x'])->assertExitCode(3);
});

it('reads a value with env:get (and a default)', function () {
    $this->bindEnv("APP_NAME=Acme\n");

    $this->artisan('laranail::env-kit.get', ['key' => 'APP_NAME'])->expectsOutput('Acme')->assertExitCode(0);
    $this->artisan('laranail::env-kit.get', ['key' => 'MISSING', '--default' => 'fallback'])
...
it('removes a key with env:unset', function () {
    ...
    $this->artisan('laranail::env-kit.unset', ['key' => 'A'])
...
it('renames a key with env:rename', function () {
    ...
    $this->artisan('laranail::env-kit.rename', ['from' => 'OLD', 'to' => 'NEW'])
...
it('lists keys, and masks secret values unless --reveal', function () {
    $this->bindEnv("APP_NAME=Acme\nDB_PASSWORD=" . envkit_canary() . "\n");

    $this->artisan('laranail::env-kit.keys')->expectsOutputToContain('APP_NAME')->assertExitCode(0);
    ...
    $this->artisan('laranail::env-kit.list', ['--reveal' => true])
        ->expectsOutputToContain(envkit_canary())
        ->assertExitCode(0);
});
...
it('env:set splits KEY=VALUE on the first equals only and confirms the write', function () {
    ...
    $this->artisan('laranail::env-kit.set', ['key' => 'URL=https://ex.test/?a=1'])
...
it('env:set with a bare KEY writes an empty value', function () {
    ...
    $this->artisan('laranail::env-kit.set', ['key' => 'EMPTYK'])->assertExitCode(0);
...
it('env:set --export writes the export prefix', function () {
    ...
    $this->artisan('laranail::env-kit.set', ['key' => 'EX', 'value' => '1', '--export' => true])->assertExitCode(0);
...
it('operates on an alternate file with --file and falls back to the default for an empty --file', function () {
    ...
    $this->artisan('laranail::env-kit.list', ['--file' => $alt])
        ->expectsOutputToContain('B=2')
        ->doesntExpectOutputToContain('A=1')
        ->assertExitCode(0);

    $this->artisan('laranail::env-kit.list', ['--file' => ''])
...
it('blocks env:set in production without --force-production and allows it with the flag', function () {
    ...
    $this->artisan('laranail::env-kit.set', ['key' => 'A', 'value' => '2'])
    ...
    $this->artisan('laranail::env-kit.set', ['key' => 'A', 'value' => '2', '--force-production' => true])->assertExitCode(0);
...
it('env:export honours --format, falls back to json for an empty one, and reports --output writes', function () {
    ...
    $this->artisan('laranail::env-kit.export', ['--format' => 'csv'])
        ->expectsOutputToContain('KEY,VALUE')
        ->assertExitCode(0);

    $this->artisan('laranail::env-kit.export', ['--format' => ''])
        ->expectsOutputToContain('"A"')
        ->doesntExpectOutputToContain('KEY,VALUE')
        ->assertExitCode(0);

    $out = dirname($path) . '/export.json';
    $this->artisan('laranail::env-kit.export', ['--output' => $out])
    ...
    $this->artisan('laranail::env-kit.export', ['--output' => ''])
        ->expectsOutputToContain('"A"')
...
it('env:import applies a json source and reports it', function () {
    ...
    $this->artisan('laranail::env-kit.import', ['source' => $src])
...
it('env:generate honours --bytes and writes the value with --set', function () {
    ...
    $this->artisan('laranail::env-kit.generate', ['--bytes' => '8', '--set' => 'TOKEN'])
...
it('env:generate prints the value when --set is empty, without writing anything', function () {
    ...
    expect(Artisan::call('laranail::env-kit.generate', ['--bytes' => '8', '--set' => '']))->toBe(0)
...
it('env:check and env:sync honour --example and fall back to the sibling example when empty', function () {
    ...
    $this->artisan('laranail::env-kit.check', ['--example' => $custom])
        ->expectsOutputToContain('NEWKEY')
        ->assertExitCode(3);

    // an empty --example falls back to the sibling .env.example
    $this->artisan('laranail::env-kit.check', ['--example' => ''])
        ->expectsOutputToContain('SIBKEY')
        ->assertExitCode(3);

    // sync honours --example…
    $this->artisan('laranail::env-kit.sync', ['--example' => $custom])
    ...
    $this->artisan('laranail::env-kit.sync', ['--example' => ''])

...
it('records changes and reads them back most-recent-first', function () {
    ...
    config([
        ...
        'laranail.env-kit.audit.path'    => $dir . '/audit.log',
        'laranail.env-kit.audit.enabled' => true,
...
it('the env:history command tables changes and reports an empty log', function () {
    ...
    $this->artisan('laranail::env-kit.history')
        ->expectsOutputToContain('Keys changed')
        ->expectsOutputToContain('NEW_FLAG') // shows the key NAME, not an index
        ->assertExitCode(0);

    config(['laranail.env-kit.audit.path' => $dir . '/empty.log']);
    $this->artisan('laranail::env-kit.history')->expectsOutputToContain('No audit history')->assertExitCode(0);
});
...
it('the env:docs command prints markdown and writes to a file', function () {
    ...
    $this->artisan('laranail::env-kit.docs')->expectsOutputToContain('Environment schema')->assertExitCode(0);

    $out = sys_get_temp_dir() . '/envkit-docs-' . bin2hex(random_bytes(5)) . '.md';
    $this->artisan('laranail::env-kit.docs', ['--output' => $out])->expectsOutputToContain('Wrote')->assertExitCode(0);
    expect((string) file_get_contents($out))->toContain('`A`');
    @unlink($out);

    // an empty --output still streams to stdout instead of writing a file
    $this->artisan('laranail::env-kit.docs', ['--output' => ''])
...
it('env:history renders when, actor and changed keys, tolerating malformed entries', function () {
    ...
    config(['laranail.env-kit.path' => $dir . '/.env', 'laranail.env-kit.audit.path' => $log]);

    expect(Artisan::call('laranail::env-kit.history'))->toBe(0);
...
it('env:history defaults to exactly 20 entries when --limit is not numeric', function () {
    ...
    expect(Artisan::call('laranail::env-kit.history', ['--limit' => 'lots']))->toBe(0);
...
it('env:history honours a numeric --limit, newest first', function () {
    ...
    expect(Artisan::call('laranail::env-kit.history', ['--limit' => '2']))->toBe(0);
